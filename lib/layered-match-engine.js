...
function parentRanking(left, right) {
  return Number(right.strongCoveredSubqueries?.length ?? 0) - Number(left.strongCoveredSubqueries?.length ?? 0)
    || Number(right.coveredSubqueries?.length ?? 0) - Number(left.coveredSubqueries?.length ?? 0)
    || right.exactAnchors - left.exactAnchors
    || right.lexicalRelevance - left.lexicalRelevance
    || right.averageTopChildScore - left.averageTopChildScore
...
function lastResultStatus(result) {
  ...
  const summarizeParent = (parent) => ({
    id: parent.id,
    documentId: parent.documentId,
    turn: parent.turn,
    bestDenseScore: parent.bestDenseScore,
    averageTopChildScore: parent.averageTopChildScore,
    lexicalRelevance: parent.lexicalRelevance,
    effectiveRelevance: parent.effectiveRelevance,
    exactAnchors: parent.exactAnchors,
    matchedSubqueries: parent.matchedSubqueries,
    strongCoveredSubqueries: parent.strongCoveredSubqueries,
    coverageBySubquery: parent.coverageBySubquery,
    coverageDecision: parent.coverageDecision ?? null,
    sourceValidation: parent.sourceValidation,
  })
  return {
    query: result.query,
    ...
    recallDiagnostics: result.recallDiagnostics,
    bestRawCandidate: result.bestRawCandidate,
    selected: result.selected.map(summarizeParent),
    candidates: result.candidates.map(summarizeParent),
  }
}

function unique(values) { return [...new Set(values)] }

function centeredExcerpt(text, probes = [], maxCharacters = 1200) {
  const source = String(text ?? '').trim()
  if (source.length <= maxCharacters) return { text: source, truncated: false }
  const normalized = source.toLowerCase()
  const anchors = probes.map((value) => String(value ?? '').trim().toLowerCase()).filter((value) => value.length >= 2)
  const positions = anchors.map((value) => normalized.indexOf(value)).filter((index) => index >= 0)
  const center = positions.length ? Math.min(...positions) : 0
  const start = Math.max(0, Math.min(source.length - maxCharacters, center - Math.floor(maxCharacters * 0.30)))
  const end = Math.min(source.length, start + maxCharacters)
  return {
...
export class LayeredMatchEngine {
  constructor({ ledger, bank, vectorEncoder = null, sourceReader = null, config = {} }) {
    ...
    this.config = {
      ...
      relativeScoreWindow: Math.max(0, Number(config.relativeScoreWindow ?? 0.20)),
      ...
      coverageLexicalMinimum: Number(config.coverageLexicalMinimum ?? 0.35),
      coverageDenseMinimum: Number(config.coverageDenseMinimum ?? 0.75),
      coverageDenseMargin: Number(config.coverageDenseMargin ?? 0.025),
      lexicalGuaranteePerQuery: Math.max(0, config.lexicalGuaranteePerQuery ?? 4),
      matchedChildExcerptCharacters: Math.max(240, Number(config.matchedChildExcerptCharacters ?? 1000)),
  ...
  #recallChildren(queryPlan, records, queryVectors) {
    ...
    for (let queryIndex = 0; queryIndex < queryPlan.allQueries.length; queryIndex += 1) {
      ...
      const runnerUpScore = rows[1]?.score ?? 0
      const admitted = []
      for (let index = 0; index < rows.length && admitted.length < this.config.childTopK; index += 1) {
        ...
        const denseMargin = index === 0 ? Math.max(0, row.score - runnerUpScore) : 0
        const strongCoverage = row.lexical.exactPhrase
          || row.lexical.coverage >= this.config.coverageLexicalMinimum
          || (index === 0 && row.dense >= this.config.coverageDenseMinimum && denseMargin >= this.config.coverageDenseMargin)
        const coverageStrength = row.lexical.exactPhrase ? 1
          : row.lexical.coverage >= this.config.coverageLexicalMinimum ? Math.max(0.55, row.lexical.coverage)
            : strongCoverage ? Math.min(0.90, 0.60 + denseMargin * 4)
              : index === 0 ? 0.20 : 0
        admitted.push({
          ...row,
          admissionReasons: reasons,
          coverageEligible: strongCoverage || index === 0,
          coverageTier: strongCoverage ? 'strong' : 'weak-top1-fallback',
          coverageStrength,
          denseMargin,
        })
      }
      const admittedParents = new Set(admitted.map((hit) => hit.parent.id))
      const lexicalCandidates = rows
        .filter((row) => row.lexical.exactAnchor && !admittedParents.has(row.parent.id))
        .sort((left, right) => Number(right.lexical.exactPhrase) - Number(left.lexical.exactPhrase)
          || right.lexical.coverage - left.lexical.coverage
          || right.lexical.strongMatchedTokens.length - left.lexical.strongMatchedTokens.length
          || right.dense - left.dense)
      const lexicalGuarantees = []
      for (const row of lexicalCandidates) {
        if (admittedParents.has(row.parent.id)) continue
        lexicalGuarantees.push({
          ...row,
          admissionReasons: ['lexical-guarantee'],
          coverageEligible: row.lexical.coverage >= 0.20 || row.lexical.strongMatchedTokens.length >= 2,
          coverageTier: 'strong',
          coverageStrength: Math.max(0.50, row.lexical.coverage),
          denseMargin: 0,
        })
        admittedParents.add(row.parent.id)
        if (lexicalGuarantees.length >= this.config.lexicalGuaranteePerQuery) break
      }
      admitted.push(...lexicalGuarantees)
      ...
      diagnostics.push({
        ...
        topScore,
        runnerUpScore,
        rows: rows.slice(0, this.config.childTopK).map((row, index) => {
          const accepted = admitted.find((hit) => hit.parent.id === row.parent.id && hit.child.childId === row.child.childId)
          return {
            rank: index + 1,
            ...
            score: row.score,
            dense: row.dense,
            lexicalScore: row.lexical.score,
            lexicalCoverage: row.lexical.coverage,
            exactPhrase: row.lexical.exactPhrase,
            admitted: Boolean(accepted),
            admissionReasons: accepted?.admissionReasons ?? [],
            coverageTier: accepted?.coverageTier ?? null,
          }
        }),
        lexicalGuarantees: lexicalGuarantees.map((row) => ({
          parentId: row.parent.id,
          childId: row.child.childId,
          score: row.score,
          dense: row.dense,
          lexicalScore: row.lexical.score,
          lexicalCoverage: row.lexical.coverage,
          strongMatchedTokens: row.lexical.strongMatchedTokens,
        })),
      })
    }
    const childHits = [...union.values()]
      .sort((left, right) => {
        const priority = (item) => Math.max(...[...item.byQuery.values()].map((hit) => hit.score + hit.coverageStrength * 0.15 + hit.lexical.coverage * 0.20))
        return priority(right) - priority(left)
      })
      .slice(0, this.config.childUnionLimit)
    return { parentMeta, childHits, recallGuardCount, diagnostics }
  }
  ...
  #aggregateParents(queryPlan, recall) {
    ...
    return [...grouped.values()].map(({ parent, childHits }) => {
      ...
      const coverageBySubquery = Object.fromEntries(matchedSubqueries.map((id) => {
        const options = hits.filter((hit) => hit.coverageEligible && hit.query.id === id)
          .sort((left, right) => right.coverageStrength - left.coverageStrength || right.score - left.score)
        const best = options[0]
        return [id, {
          strength: best?.coverageStrength ?? 0,
          tier: best?.coverageTier ?? 'none',
          score: best?.score ?? 0,
          dense: best?.dense ?? 0,
          lexicalCoverage: best?.lexical.coverage ?? 0,
          denseMargin: best?.denseMargin ?? 0,
        }]
      }))
      const strongCoveredSubqueries = matchedSubqueries.filter((id) => coverageBySubquery[id]?.tier === 'strong')
      ...
      return {
        ...
        childHits: childHits.map((item) => {
          const queryHits = [...item.byQuery.values()]
          const probes = unique(queryHits.flatMap((hit) => [
            ...(hit.lexical.matchedTokens ?? []),
            hit.lexical.exactPhrase ? hit.query.text : null,
          ]).filter(Boolean)).sort((left, right) => String(right).length - String(left).length)
          const fullText = childText(parent, item.child)
          const excerpt = centeredExcerpt(fullText, probes, this.config.matchedChildExcerptCharacters)
          return {
            ...
            matchedQueryText: unique(queryHits.map((hit) => hit.query.text)),
            matchedTokens: unique(queryHits.flatMap((hit) => hit.lexical.matchedTokens ?? [])),
            bestScore: Math.max(...queryHits.map((hit) => hit.score), 0),
            ...
            coverage: Object.fromEntries([...item.byQuery].map(([id, hit]) => [id, {
              tier: hit.coverageTier,
              strength: hit.coverageStrength,
              denseMargin: hit.denseMargin,
            }])),
          }
        }),
        matchedSubqueries,
        coveredSubqueries: matchedSubqueries.filter((id) => id !== 'S0'),
        strongCoveredSubqueries: strongCoveredSubqueries.filter((id) => id !== 'S0'),
        coverageBySubquery,
        bestDenseScore,
  ...
  #selectCoverage(queryPlan, parents) {
    ...
    const redundancyRejected = new Set()
    const selectionLimit = this.config.parentSelectLimit
    const minimumParentCount = globalOnly ? selectionLimit : Math.min(selectionLimit, Math.max(2, targets.length))
    while (selected.length < selectionLimit && remaining.length > 0
      && (globalOnly || covered.size < targets.length || selected.length < minimumParentCount)) {
      let best = null
      for (const parent of remaining) {
        const coverageOptions = targets
          .filter((id) => parent.matchedSubqueries.includes(id) && !covered.has(id))
          .map((id) => ({ id, ...(parent.coverageBySubquery?.[id] ?? { strength: 0, tier: 'none' }) }))
          .sort((left, right) => Number(right.tier === 'strong') - Number(left.tier === 'strong') || right.strength - left.strength || left.id.localeCompare(right.id))
        const selectedCoverage = coverageOptions
        const newCoverage = selectedCoverage.map((item) => item.id)
        if (!globalOnly && newCoverage.length === 0 && selected.length >= minimumParentCount) continue
        ...
        const coverageValue = selectedCoverage.reduce((sum, item) => sum + (item.tier === 'strong' ? 2 : 0.65) * Math.max(0.20, item.strength), 0)
        const value = (globalOnly ? (selected.length === 0 ? 1 : 0.30) : coverageValue + (newCoverage.length ? 0 : 0.25))
          + parent.bestDenseScore
          + Math.min(0.45, parent.exactAnchors * 0.15)
          + (sourceTurns.has(sourceKey) ? 0 : 0.15)
          - redundancy * 0.75
        if (!best || value > best.value || (value === best.value && parentRanking(parent, best.parent) < 0)) {
          best = { parent, value, newCoverage, selectedCoverage, redundancy, sourceKey }
        }
      }
      ...
      selected.push({
        ...
        coverageDecision: {
          ...
          coverage: best.selectedCoverage,
    ...
    for (const item of remaining) {
      if (selected.some((chosen) => semanticRedundancy(item, chosen, this.config.stopwords) > 0.88)) redundancyRejected.add(item.id)
    }
    return {
      selected,
      coveredSubqueries: globalOnly && selected.length ? ['S0'] : [...covered],
      uncoveredSubqueries: targets.filter((id) => !covered.has(id)),
      redundancyRejectedCount: redundancyRejected.size,
    }
  }
  ...
  #result(query, viewer, queryPlan, ranked, searchedBank, retrieval = {}) {
    ...
    const result = {
      ...
      retrievalMode: retrieval.mode ?? 'lexical-only',
      vectorAvailable: retrieval.vectorAvailable ?? false,
      degraded: retrieval.degraded === true,
      degradationReason: retrieval.reason ?? null,
      ...
      recallDiagnostics: ranked.recall.diagnostics,
  ...
  #retrieveWithVectors(query, viewer, queryPlan, queryVectors, retrieval = {}) {
    ...
    if (retrieval.degraded) this.stats.lexicalFallbackQueries += 1
    return this.#result(query, viewer, queryPlan, ranked, searchedBank, retrieval)
  }
  ...
  retrieve(query, viewer = {}) {
    ...
    const available = queryVectors.some(Boolean)
    const mode = status.provider === 'feature-hash' ? 'feature-hash+lexical' : available ? 'hybrid' : 'lexical-only'
    return this.#retrieveWithVectors(query, viewer, queryPlan, queryVectors, { mode, vectorAvailable: available, degraded: false })
  }
  ...
  async retrieveAsync(query, viewer = {}) {
    ...
    const status = this.vectorEncoder?.status?.() ?? {}
    const available = queryVectors.some(Boolean)
    const configuredVector = status.provider && !['none', 'feature-hash'].includes(status.provider)
    const degraded = configuredVector && !available && Number(status.failures ?? 0) > 0
    const mode = status.provider === 'feature-hash' ? 'feature-hash+lexical'
      : available ? 'hybrid'
    ...
    return this.#retrieveWithVectors(query, viewer, queryPlan, queryVectors, {
      ...
      degraded,
      reason: degraded ? status.lastError ?? 'vector-unavailable' : null,
    })
  }
  ...
  renderRetrievalHint(selected = []) {
    const count = selected.filter((item) => item?.qualified !== false).length
    if (!count) return null
    const prompt = `（检索提示：当前问题命中 ${count} 个已卸载且来源已验证的 Parent，但完整目录超出本轮证据预算。请先调用 sensory_recall，直接读取 matchedChildren；只有缺少相邻上下文时再调用 sensory_open。工具返回 converged 或 budgetExceeded 后立即根据已有证据作答。）`
    return { prompt, entryCount: count, estimatedTokens: estimateTokens(prompt) }
  }
