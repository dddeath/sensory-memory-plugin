...
export class MemoryPolicy {
  ...
  shouldMoveWorkingToSensory(segment, { currentTurn, contextPressure = false } = {}) {
    ...
    if (contextPressure) return true
    const last = this.strongAssociations(segment).sort((a, b) => b.turn - a.turn)[0]?.turn ?? segment.turn
    return Number(currentTurn) - Number(last) >= this.config.workingInactiveTurns
  }
