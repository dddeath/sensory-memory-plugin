export class SensoryMaintenance {
  ...
  async prepareTurnMeasurement(agent, { turn, signal = null } = {}) {
    if (!agent?.session) throw new TypeError('measurement agent session is required')
    const decision = await this.runtime.preStep({
      agent,
      messages: [],
      turn: Number(turn ?? 0),
      step: 1,
      signal,
    }, async () => ({ kind: 'skip', reason: 'benchmark-turn-measurement' }))
    return {
      sessionId: String(agent.session.id),
      turn: Number(turn ?? 0),
      decisionKind: decision?.kind ?? null,
      layered: this.runtime.status(agent.session.id),
    }
  }
