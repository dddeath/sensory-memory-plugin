...
export function installChunkMemory(ctx, config, services) {
  ...
  const contextText = toolMode === 'compression-only'
    ...
    : toolMode === 'retrieval-only'
      ? `本环境使用压力驱动的 Parent/Child 上下文压缩：低于上下文阈值时保留原始工作消息，不自动召回；达到阈值后才把冷 Parent 无损卸载到当前会话索引。回答当前问题时只可使用 sensory_recall 检索和 sensory_open 展开已卸载 Parent。${retrievalToolCallLimit ? `每题最多调用 sensory_recall ${retrievalToolCallLimit} 次、sensory_open ${retrievalToolCallLimit} 次，随后直接作答。` : ''}`
      : '本环境使用压力驱动的 Parent/Child 上下文压缩：低于上下文阈值时保留原始工作消息，不自动召回；达到阈值后才把冷 Parent 无损卸载到当前会话索引，Child 仅提供 E5 定位，后续只按需展开已卸载 Parent。DSH 原生 compaction 保留为安全兜底。可用 sensory_recall/open/store/demote/status、memory_layer_status、memory_bank_open、memory_forget；调试工具可输出压力、Parent/Child、来源与完整 prompt。'
