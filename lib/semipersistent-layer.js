...
export class SemipersistentLayer {
  ...
  // Compatibility facade: passive matches no longer promote memory.
  onHit(entityId, viewer = {}) {
    return { entityId, scopeId: String(viewer.sessionId ?? viewer.scopeId ?? ''), cached: false, passive: true, associationWeight: 0 }
  }
  ...
  status(sessionId = null, workspaceId = null) {
    ...
    return {
      ...
      entries: projections.map((projection) => ({
        entityId: projection.recordId,
        entity: records.find((record) => record.id === projection.recordId)?.title ?? projection.recordId,
        ...projection,
      })),
