...
function normalizePlan(value) {
  return {
    ...
    chunkHints: Array.isArray(value?.chunkHints) ? value.chunkHints.map(String).slice(0, 8) : [],
    timeConstraint: {
...
export class MemoryRetrievalPlanner {
  ...
  async plan(result, viewer = {}) {
    if (result.sufficient || !result.needsPlanner || !this.enabled) return null
    const stepKey = `${viewer.sessionId}:${viewer.turn ?? 0}`
    ...
    try {
      if (!plan) {
        ...
        const candidates = result.candidates.slice(0, 8).map((item) => ({
          id: item.id,
          layer: item.layer,
          label: item.label,
        ...
        const output = await this.llm.complete(
          `Resolve the query into a chunk retrieval plan. Select only IDs in candidates. Do not answer the question. Return one JSON object with resolvedQuery, chunkHints, timeConstraint:{kind,current|asOf|range,from,to}, selectedCandidateIds, needOpen, confidence.\nQuery:${result.query}\nReasons:${result.slowPathReasons.join(',')}\nCandidates:${JSON.stringify(candidates)}`,
          { system: 'Choose existing context chunk IDs. Never create or rewrite memory content.', maxTokens: 192, purpose: 'memory-retrieval-plan', sessionId: viewer.sessionId, reasoningEffort: 'off' },
        )
      ...
      const verified = await this.matcher.verifySelected(result, plan.selectedCandidateIds, plan)
