...
export class LayeredMemoryRuntime {
  ...
  async preStep(payload, next) {
    ...
    const transitions = drain.timeout ? [] : this.#processTransitions(agent.session, turn, priorBudget, model)
    ...
    const rootManifest = reconcile.restoreSensoryManifest ? this.#rootManifest(sessionId) : null
