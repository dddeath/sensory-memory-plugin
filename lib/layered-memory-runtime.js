...
export class LayeredMemoryRuntime {
  ...
  #extractSegmentMetadata(segment) {
    ...
    const retrieval = buildRetrievalFeatures(extractionText, {
      entities,
      sourceRefs: canonicalSourceRefs(segment, this.config.trustedEvidenceTools),
    })
    const title = retrieval.title ?? entities[0]?.name ?? `session-${segment.sessionId}-turn-${segment.turn}`
    return {
      ...segment,
      title,
      entities,
  ...
  #sensoryEntriesFor(segment) {
    const entities = segment.entities?.length ? segment.entities : [{ name: segment.title, observations: [], aliases: segment.retrievalAliases ?? [], checkpointOnly: true }]
    return entities.filter(meaningfulEntity).map((entity, index) => ({
      ...
      aliases: [...new Set([...(entity.aliases ?? []), ...(index === 0 ? segment.retrievalAliases ?? [] : [])])]
        .filter((alias) => alias !== titleFor(segment, entity, index)).slice(0, 8),
      retrievalFeatureVersion: segment.retrievalFeatureVersion ?? 1,
      retrievalTerms: clone(segment.retrievalTerms ?? []),
      evidenceSourceRefs: clone(segment.evidenceSourceRefs ?? []),
  ...
  #mirrorSensory(entry) {
    ...
    const id = this.index.addEntity({
      ...
      keywords: (entry.retrievalTerms ?? []).map((term) => term.value),
    })
  ...
  async preStep(payload, next) {
    ...
    const slowStatus = slow ? {
      ...
      verifiedSelected: (slow.selected ?? []).map((item) => ({
        id: item.id,
        title: item.title,
        baseRelevance: item.baseRelevance,
        sourceEvidenceScore: item.sourceEvidenceScore,
