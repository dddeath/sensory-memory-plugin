...
export class LayeredMemoryRuntime {
  ...
  #processTransitions(session, currentTurn, budget, model = null) {
    ...
    const working = this.#segments(session.id)
      .filter((segment) => segment.state === 'working')
      .map((segment) => ({ segment, priority: this.policy.compressionPriority(segment, { currentTurn }) }))
      .filter((item) => item.priority.eligible)
      .sort((left, right) => Number(right.priority.cold) - Number(left.priority.cold)
        ...
        || right.priority.estimatedTokens - left.priority.estimatedTokens
        || Number(left.segment.firstSeq) - Number(right.segment.firstSeq))
      .map((item) => item.segment)
    let currentBudget = budget
    for (const segment of working) {
      const result = this.#transitionWorking(session, segment, currentTurn, { ...currentBudget, pressureTriggered: true })
  ...
  async preStep(payload, next) {
    ...
    const postTransitionBudget = this.surfaceProjector.budget({
      sessionId,
      session: agent.session,
      contextWindow: model.contextWindow,
      maxOutputTokens: model.maxOutputTokens,
