...
export class LayeredMemoryRuntime {
  ...
  #pointerChunks(chunks, replaced, overrides = {}) {
    return chunks.map((chunk) => ({
      ...chunk,
      surfaceResidency: overrides.surfaceResidency ?? chunk.surfaceResidency ?? 'labeled-pointer',
      pointer: {
        ...(chunk.pointer ?? {}),
        ...(replaced?.pointer ?? {}),
        ...(overrides.pointer ?? {}),
  ...
  #compressPointers(session, budget) {
    ...
    const parents = this.ledger.list(SENSORY_LEDGER_COLLECTION, { scopeKind: 'session', scopeId: session.id })
    ...
    for (const action of plan.actions) {
      ...
      if (!parent) continue
      const rewritten = this.surfaceProjector.rewriteSensoryPointer(session, parent, action.mode)
      if (!rewritten.ok) {
      ...
      const updated = {
        ...
        surfaceLineage: [...(parent.surfaceLineage ?? []), rewritten.lineage],
        updatedAt: Date.now(),
      }
      this.ledger.upsert(SENSORY_LEDGER_COLLECTION, updated, { scopeKind: 'session', scopeId: session.id, id: parent.id })
  ...
  async preStep(payload, next) {
    ...
    let postTransitionBudget = this.surfaceProjector.budget({
      ...
      contextWindow: model.contextWindow,
      maxOutputTokens: model.maxOutputTokens,
      request: { messages: [...(agent.session.deriveMessages?.() ?? []), ...claimed] },
    })
    ...
    const injectionLimit = Math.floor(postTransitionBudget.usableInputTokens * this.surfaceProjector.config.contextPressureRatio)
    ...
    const rendered = this.surfaceProjector.renderMessages({ decision, claimed, sessionId, turn, step, semi, catalog })
  ...
  layerCounts(sessionId, workspaceId = this.sessionState(sessionId).workspaceId) {
    ...
    const surfaceResidency = {
      labeledPointer: parents.filter((parent) => parent.surfaceResidency === 'labeled-pointer').length,
      compactPointer: parents.filter((parent) => parent.surfaceResidency === 'compact-pointer').length,
      idPointer: parents.filter((parent) => parent.surfaceResidency === 'id-pointer').length,
      ...
      estimatedPointerTokens: parents.reduce((sum, parent) => sum + Math.max(0, Number(parent.pointer?.estimatedTokens ?? 0)), 0),
    }
    return {
