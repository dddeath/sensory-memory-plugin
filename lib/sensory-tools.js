    ...
    execute: async ({ chunk }, exec = {}) => {
      ...
      if (runtime) {
        ...
        if (sensory) {
          ...
          const disclosed = matchedChildren.length
            ? clipText(matchedChildren.map((child) => child.excerpt).join('\n\n'), 6000)
          ...
          return json({
            ...
            matchedChildren,
            disclosure: {
              mode: matchedChildren.length ? 'matched-child-context' : 'bounded-parent-view',
