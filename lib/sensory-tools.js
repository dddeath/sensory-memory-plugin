...
export function createSensoryToolDefinitions({
  ...
  const recall = defineTool({
    name: 'sensory_recall',
    description: '主动检索当前会话已卸载的 context chunk；只读且不增加关联。',
    parameters: {
  ...
  const open = defineTool({
    name: 'sensory_open',
    description: '按 chunk ID 展开已卸载上下文的完整原文，并记录一次显式强关联。',
    ...
    execute: async ({ chunk }, exec = {}) => {
      ...
      if (runtime) {
        ...
        if (sensory) {
          ...
          return json({ chunkId: sensory.id, label: sensory.label, layer: 'sensory', found: true, coreText: sensory.coreText, contextText: sensory.contextText, vector: sensory.vector ? { provider: sensory.vector.provider, model: sensory.vector.model, dimensions: sensory.vector.dimensions } : null, sources, associationWeight: 1 })
        }
  ...
  const store = defineTool({
    name: 'sensory_store',
    description: '把文本按结构切成 context chunk，并为每个 chunk 写入一个向量编码。',
  ...
  const status = defineTool({
    ...
    execute: async (_args, exec = {}) => {
      ...
      return json({ architecture: 'chunk-only-vector', available: false, reason: 'chunk-runtime-unavailable' })
    },
  })
  ...
  if (cache) {
    tools.push(defineTool({
      ...
      description: '查看半持久 context chunk 投影、预算和向量检索慢路径统计。',
      parameters: {},
      isConcurrencySafe: () => true,
