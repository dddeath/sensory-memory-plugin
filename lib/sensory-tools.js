...
export function createSensoryToolDefinitions({
  ...
  const scopeForExec = (exec = {}) => matcher.scopeFor?.({
  ...
  const demote = defineTool({
    ...
    isConcurrencySafe: () => false,
    execute: async ({ sourceSeq }, exec = {}) => json(await demoter.demoteBySeq(
      sourceSeq,
  ...
  if (auditor) {
    tools.push(defineTool({
      ...
      isConcurrencySafe: () => false,
      execute: async ({ sampleSize = 20 }, exec = {}) => {
        const scopeId = scopeForExec(exec)
