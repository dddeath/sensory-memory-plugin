...
const CATALOG_PURPOSES = new Set([
  'sensory-catalog', 'sensory-catalog-legacy', 'sensory-retrieval-hint', 'sensory-checkpoint', 'sensory-root-manifest',
...
function parentRecordForDebug(parent) {
  ...
  return {
    ...
    lastSeq: parent.lastSeq,
    tokenCount: parent.tokenCount,
    evidenceQuality: parent.evidenceQuality,
    verifiedSource: parent.verifiedSource,
    temporalCurrent: parent.temporalCurrent !== false,
    sourceRefs: clone(parent.sourceRefs ?? []),
    vectorSpec: clone(parent.vectorSpec ?? vectorMetadata(parent.vector)),
    vector: vectorMetadata(parent.vector),
    currentView: renderCurrentParentView(parent),
    supersededRanges: clone(parent.supersededRanges ?? []),
    childSpans: (parent.childSpans ?? []).map((child) => ({
      childId: child.childId,
      startOffset: child.startOffset,
      endOffset: child.endOffset,
      tokenCount: child.tokenCount,
      headingPath: clone(child.headingPath ?? []),
      embeddingTextPreview: child.embeddingTextPreview,
      temporalCurrent: child.temporalCurrent !== false,
      supersededBy: child.supersededBy ?? null,
      supersedes: clone(child.supersedes ?? []),
      vector: vectorMetadata(child.vector),
    })),
  }
}

function matchForDebug(result) {
  if (!result) return null
  return {
    query: result.query,
    queryPlan: clone(result.queryPlan),
    sufficient: result.sufficient,
    needsPlanner: result.needsPlanner,
    slowPathReasons: clone(result.slowPathReasons),
    ...
    coveredSubqueries: clone(result.coveredSubqueries),
    uncoveredSubqueries: clone(result.uncoveredSubqueries),
    recallGuardCount: result.recallGuardCount,
    redundancyRejectedCount: result.redundancyRejectedCount,
    parents: (result.candidates ?? []).map((parent) => ({
      id: parent.id,
      label: parent.label,
      ...
      lowConfidence: parent.lowConfidence,
      qualified: parent.qualified,
      matchedSubqueries: clone(parent.matchedSubqueries),
      admissionReasons: clone(parent.admissionReasons),
      childHits: clone(parent.childHits),
      sourceValidation: clone(parent.sourceValidation),
      coverageDecision: clone(parent.coverageDecision),
    })),
  }
}

function semipersistentRecordForDebug(record) {
  if (!record) return null
  const parents = record.contextChunks ?? record.segment?.contextChunks ?? []
  return {
    id: record.id,
    ...
    state: record.state,
    sourceSeqs: clone(record.sourceSeqs ?? []),
    importance: record.importance,
    durability: record.durability,
    evidenceQuality: record.evidenceQuality,
    associations: clone(record.associations ?? []),
    parentCount: parents.length,
    parents: parents.map(parentRecordForDebug),
...
function parseCatalog(text) {
  ...
  for (const line of String(text ?? '').split(/\r?\n/)) {
    const parent = line.match(/^\s*##\s+\[\[([^\]]+)\]\]\s+\[parent\]\s+\[([^\]]+)\]\s+\[seq\s+([^\]]+)\]/)
    if (parent) {
      entries.push({ line, cache: parent[2] === 'cache', name: parent[1].trim(), seq: parent[3].trim(), layer: parent[2].trim(), summary: 'Parent current view' })
      continue
    }
...
export class SensoryDebugService {
  ...
  cachePrompt(exec = {}) {
    ...
    return {
      ...
      entries: (status?.entries ?? []).map((entry) => ({
        ...entry,
        record: this.runtime
          ? semipersistentRecordForDebug(this.ledger?.get?.('semipersistentRecords', entry.recordId ?? entry.chunkId, { scopeKind: 'workspace', scopeId: entry.workspaceId }))
          : null,
  ...
  indexPrompt(exec = {}) {
    ...
    const catalog = this.#catalogContext(exec)
    const promptEntries = catalog.entries.filter((entry) => !entry.cache)
    const parents = this.ledger?.list?.('sensoryChunks', { scopeKind: 'session', scopeId }) ?? []
    return {
      schemaVersion: 1,
      ...
      sessionId: String(exec?.agent?.session?.id ?? ''),
      scopeId,
      available: Boolean(catalog.text || parents.length),
      prompt: promptEntries.map((entry) => entry.line).join('\n') || null,
      fullCatalogPrompt: catalog.text,
      entries: promptEntries.map((entry) => ({
        ...entry,
        record: this.runtime
          ? parentRecordForDebug(parents.find((item) => `chunk:${item.id}` === entry.name || item.id === entry.name || item.label === entry.name))
          : null,
      })),
      parents: parents.map(parentRecordForDebug),
      attributes: {
        ...
        chunkStore: {
          ...
          chunkCount: parents.length,
          parentCount: parents.length,
          activeParentCount: parents.filter((parent) => parent.state === 'active').length,
          pendingParentCount: parents.filter((parent) => parent.state === 'pending-vector').length,
          childSpanCount: parents.reduce((sum, parent) => sum + (parent.childSpans?.length ?? 0), 0),
          averageChildrenPerParent: parents.length ? parents.reduce((sum, parent) => sum + (parent.childSpans?.length ?? 0), 0) / parents.length : 0,
          supersededRangeCount: parents.reduce((sum, parent) => sum + (parent.supersededRanges?.length ?? 0), 0),
          sourceStoreDir: this.ledger?.rootDir ?? null,
        },
        matcher: this.matcher?.status?.() ?? null,
        lastMatch: matchForDebug(this.matcher?.lastResult ?? null),
        catalogMessage: catalog.message ? describeMessage(catalog.message.message, catalog.message.index) : null,
        promptEntryCount: promptEntries.length,
        promptEstimatedTokens: estimateTokens(promptEntries.map((entry) => entry.line).join('\n')),
        layered: this.runtime?.status?.(scopeId) ?? null,
