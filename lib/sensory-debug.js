...
function scopeFrom({ matcher, config, runtime }, exec = {}) {
  if (runtime) return String(exec?.agent?.session?.id ?? '')
...
export class SensoryDebugService {
  ...
  cachePrompt(exec = {}) {
    ...
    return {
      ...
      entries: (status?.entries ?? []).map((entry) => ({
        ...entry,
        record: this.runtime
          ? this.ledger?.get?.('semipersistentRecords', entry.recordId ?? entry.entityId, { scopeKind: 'workspace', scopeId: entry.workspaceId })
          : this.index.get(entry.entityId, scopeId),
  ...
  indexPrompt(exec = {}) {
    ...
    return {
      ...
      entries: promptEntries.map((entry) => ({
        ...entry,
        record: this.runtime
          ? this.ledger?.list?.('sensoryEntries', { scopeKind: 'session', scopeId }).find((item) => item.title === entry.name)
          : this.index.getEntityByName(entry.name, scopeId),
      })),
      attributes: {
  ...
  async clearWorkspaceIndex(exec = {}, { confirm = false } = {}) {
    ...
    const scopeId = scopeFrom(this, exec)
    const mode = this.runtime ? 'session' : (this.config.indexScope === 'session' ? 'session' : 'global')
    const base = {
      schemaVersion: 1,
      ...
      deprecatedAlias: Boolean(this.runtime),
    }
