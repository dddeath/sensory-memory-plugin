...
export class MemorySurfaceProjector {
  ...
  replaceSegment(session, segment, { purpose = 'sensory-checkpoint', text, expectedRevision = segment.surfaceRevision ?? 0, transition = 'working-to-sensory' } = {}) {
    ...
    const event = session.append('user/message', message, {
      ...
      sourceEventSeqs: [...(segment.sourceSeqs ?? [])],
    })
    const lineage = {
      transition,
      purpose,
  ...
  prepareSemipersistentSnapshot(session, semi) {
    ...
    if (!semi?.prompt) {
      ...
      const event = session.append('user/message', replacement, {
        surfaceOp: { op: 'replace', start: Number(priorEvent.seq), end: Number(priorEvent.seq) },
        sourceEventSeqs: [Number(priorEvent.seq)],
      })
  ...
  reconcile(session, segments = []) {
    ...
    return {
      ...
      externalCompactionDetected: surface.some((message) => message?.source?.kind === 'plugin' && String(message?.source?.purpose ?? '').includes('compact')),
    }
  }
