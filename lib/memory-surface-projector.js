...
export function classifySurfaceTokens(request = {}, tokenMeter = null) {
  ...
  for (let index = 0; index < messages.length; index += 1) {
    ...
    else if (purpose === 'sensory-catalog' || purpose === 'sensory-retrieval-hint') rows.retrievalEvidence += cost
  ...
  return {
    ...
    headerComplete: request.headerComplete === undefined
      ? Object.hasOwn(request, 'system') && Object.hasOwn(request, 'tools')
...
export class MemorySurfaceProjector {
  constructor({ ledger, tokenMeter = null, config = {} }) {
    ...
    this.config = {
      ...
      contextPressureTargetRatio: Math.max(0.05, Math.min(0.9, config.contextPressureTargetRatio ?? 0.35)),
      ...
      effectiveInputCapTokens: Number(config.effectiveInputCapTokens) > 0
        ...
        : null,
      llmProvider: String(config.llmProvider ?? config.provider ?? '@local/sensory-memory'),
      llmModel: String(config.llmModel ?? config.model ?? 'surface-detach'),
    }
  ...
  replaceSegment(session, segment, { purpose = 'sensory-checkpoint', text, pointer = null, expectedRevision = segment.surfaceRevision ?? 0, transition = 'working-to-sensory' } = {}) {
    ...
    if (visible && (!visible.has(start) || !visible.has(end))) {
      return {
        ok: false,
        reason: 'source-range-boundary-not-visible',
        startVisible: visible.has(start),
        endVisible: visible.has(end),
      }
    }
  ...
  renderMessages({ decision, claimed = [], sessionId, turn, step, semi = null, catalog = null, retrievalHint = null }) {
    ...
    add('sensory-retrieval-hint', retrievalHint?.prompt, {
      entryCount: retrievalHint?.entryCount ?? 0,
      estimatedTokens: retrievalHint?.estimatedTokens ?? 0,
      architecture: 'parent-child-vector-v2',
    })
