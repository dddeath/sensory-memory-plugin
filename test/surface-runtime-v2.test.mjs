...
test('detached Parent replacement remains in raw events and produces zero derived Provider messages', (t) => {
  ...
  assert.equal(result.message.source.kind, 'model')
  assert.equal(result.message.source.plugin, '@local/sensory-memory')
  assert.equal(typeof result.message.source.provider, 'string')
  assert.equal(typeof result.message.source.model, 'string')
